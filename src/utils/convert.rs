use super::standard::{I_REF, P_REF, W_REF};
...
pub fn lw_2_lp(lw: Vec<f32>, r: f32, q: f32) -> Vec<f32> {
    lw.iter()
        .map(|x| x - f32::abs(10.0 * f32::log10(q / (4.0 * PI * r * r))))
        .collect()
}
...
pub fn lp_2_lw(lp: Vec<f32>, r: f32, q: f32) -> Vec<f32> {
    lp.iter()
        .map(|x| f32::abs(10.0 * f32::log10(q / (4.0 * PI * r * r))) + x)
        .collect()
}
...
pub fn lp_2_p(lp: Vec<f32>) -> Vec<f32> {
    lp.iter()
        .map(|x| f32::powf(10.0, x / 20.0) * P_REF)
        .collect()
}
...
pub fn li_2_i(li: Vec<f32>) -> Vec<f32> {
    li.iter()
        .map(|x| f32::powf(10.0, x / 10.0) * I_REF)
        .collect()
}
...
pub fn lw_2_w(lw: Vec<f32>) -> Vec<f32> {
    lw.iter()
        .map(|x| f32::powf(10.0, x / 10.0) * W_REF)
        .collect()
}
